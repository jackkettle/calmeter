...
public class FoodItemHelper {
	...
	public static void applyServingModifierToValues (FoodItem foodItem) {

		Double weightInGrams = foodItem.getWeightInGrams ();
		Double modifier = weightInGrams / 100;

		BeanWrapper individualAccessor = PropertyAccessorFactory.forBeanPropertyAccess (foodItem.getNutritionalInformation ());
		for (PropertyDescriptor propertyDescriptor : individualAccessor.getPropertyDescriptors ()) {
			...
			if (propertyDescriptor.getPropertyType ().equals (Integer.class) || propertyDescriptor.getPropertyType ().equals (int.class)) {
				...
				individualAccessor.setPropertyValue (propertyDescriptor.getName (), DoubleHelper.round (updateValue, 2).intValue ());
				continue;
			}
